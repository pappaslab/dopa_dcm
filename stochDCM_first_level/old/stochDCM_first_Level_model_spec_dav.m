...
%% Directories
dcmbase = '/home/despoC/dvogel/rsfMRI_DA_Hierarchy/rsfMRI_DA_Analysis_DAVID/fMRIprep/DA_team_rsfMRI/stochDCM/TEST';
...
for i = 1:length(subjects) 
    ...
    for s = 1:length(session)
        ...
        if exist(SPMfile,'file')==2
            fprintf('Skipping first level analysis for subject %s and session %s\n\n',sub, cond);
        else
      
            fprintf(1,'Initialising outputdir for  subject %s and session %s and now lets run first level analysis \n', sub, cond);
            mkdir(outputdir)
            ...
            if exist(preproc_file) == 2
                ...
                %fourDepi = spm_select('expand', fullfile([preproc_base, '/', subs, '/Smooth_MNI/smooth_', subs, '_task-HTB', run, '_bold_space-MNI152NLin2009cAsym_preproc_nuisreg_4D.nii']));
            else 
                %preproc_file_gzip = ([preproc_dir, '/Fourier_Filt_func_data.nii.gz']);
                preproc_file_gzip = ([preproc_dir, '/fmriprep_afni-sub-', int2str(sub), '-', cond, '.nii.gz']);
                gunzip(preproc_file_gzip);
                %preproc_data = spm_select('expand', fullfile(preproc_dir,'/Fourier_Filt_func_data.nii'));
                preproc_data = spm_select('expand', fullfile([preproc_dir, '/fmriprep_afni-sub-', int2str(sub), '-', cond, '.nii']));
            end
            ...
            if s == 1
                preproc_data_all = preproc_data_files; 
                
            else 
                preproc_data_all = [preproc_data_all; preproc_data_files]; 
            end 
            ...
            matlabbatch{1}.spm.stats.fmri_spec.sess(1).scans = preproc_data_all;
