...
subjects = [17:28, 30:39, 41:58, 60:61, 63:71, 73:74, 76:77];
...
for i = 1:length(subjects) 
    ...
    for s = 1:length(session)
        ...
        if sub<=9
            outputdir=([dcmbase, '/sub-00', int2str(sub), '/session-', cond, '/']);
            nifti_file_path = fullfile(fmriprep_preproc_data_dir, ['/sub-00', int2str(sub), '/ses-', cond]); 
        else
            outputdir=([dcmbase, '/sub-0', int2str(sub), '/session-', cond, '/']); 
            nifti_file_path = fullfile(fmriprep_preproc_data_dir, ['/sub-0', int2str(sub), '/ses-', cond]); 
        end
        ...
        if exist(SPMfile,'file')==2
            ...
            if sub <= 9
                preproc_dir = strcat([fmriprep_preproc_data_dir,'/sub-00', int2str(sub), '/ses-', cond]);
            else
                preproc_dir = strcat([fmriprep_preproc_data_dir,'/sub-0', int2str(sub), '/ses-', cond]); 
            end
