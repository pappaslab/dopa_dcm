...
%% Directories
dcmbase = '/home/despoC/dvogel/rsfMRI_DA_Hierarchy/rsfMRI_DA_Analysis_DAVID/fMRIprep/Idibrom_rsfMRI/DCM_striatum_fmriprep_afni';
fmriprep_preproc_data_dir = '/home/despoC/dvogel/rsfMRI_DA_Hierarchy/rsfMRI_DA_Analysis_DAVID/fMRIprep/Idibrom_rsfMRI/preproc';
...
centers = [-44 48 4; -38 28 44; -26 14 52; -52 20 28; -40 10 20; -24 4 54; -9 13 1; -11 6 15];
...
for i = 1:numSub %length(subjects) 
    ...
    for s = 1:length(session)
        ...
        if exist(SPMfile,'file')==2
            ...
            preproc_file = ([preproc_dir, '/fmriprep_afni-sub-0', sub, '-', cond, '.nii']);
            if exist(preproc_file) == 2
                %preproc_data = spm_select('expand', fullfile(preproc_dir,'/Fourier_Filt_func_data.nii'));
                preproc_data = spm_select('expand', fullfile([preproc_dir, '/fmriprep_afni-sub-0',sub, '-', cond, '.nii']));
            else 
                %preproc_file_gzip = ([preproc_dir, '/Fourier_Filt_func_data.nii.gz']);
                preproc_file_gzip = ([preproc_dir, '/fmriprep_afni-sub-0', sub, '-', cond, '.nii.gz']);
