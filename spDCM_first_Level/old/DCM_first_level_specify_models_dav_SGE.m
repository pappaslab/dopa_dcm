...
%%-----------------------------------------------------------------------
function DCM_first_level_specify_models_dav_SGE(subject)
%clear all; clc; close all; 

%% Directories
dcmbase = '/home/despoC/dvogel/rsfMRI_DA_Hierarchy/rsfMRI_DA_Analysis_DAVID/fMRIprep/TBP_rsfMRI/DCM_striatum_fmriprep_afni';
%fmriprep_preproc_data_dir = '/home/despoC/dvogel/rsfMRI_DA_Hierarchy/rsfMRI_DA_Analysis_DAVID/fMRIprep/DA_team_fMRIprep/data/fmriprep/fmriprep';
...
for i = 1:numSub%length(subjects) 
    ...
    for s = 1:length(session)
        ...
        if sub <= 9 
            sub_base = ([dcmbase, '/sub-00', sub, '/session-', cond, '/']); 
        else
            sub_base = ([dcmbase, '/sub-0', sub, '/session-', cond, '/']); 
        end 
        
        SPMmat = [sub_base 'DCM_sub-0', sub, '-session-', cond, '.mat'];  
        if exist(SPMmat,'file')==2
            ...
            main_model(2,1) = 1; main_model(3,2) = 1; main_model(2,3) = 1; main_model(3,4) = 1; main_model(3,5) = 1; main_model(7,1) = 1; main_model(7,3) = 1; main_model(7,5) = 1; main_model(1,7) = 1; main_model(5,7) = 1; main_model(2,8) = 1; main_model(6,8) = 1;          
            main_model(3,1) = 1; main_model(4,2) = 1; main_model(4,3) = 1; main_model(6,4) = 1; main_model(6,5) = 1; main_model(8,1) = 1; main_model(8,3) = 1; main_model(8,5) = 1; main_model(2,7) = 1; main_model(6,7) = 1; main_model(3,8) = 1; main_model(7,8) = 1;
            main_model(5,1) = 1; main_model(5,2) = 1; main_model(5,3) = 1; main_model(1,5) = 1; main_model(4,6) = 1; main_model(7,2) = 1; main_model(7,4) = 1; main_model(7,6) = 1; main_model(3,7) = 1; main_model(8,7) = 1; main_model(4,8) = 1;
            main_model(1,2) = 1; main_model(1,3) = 1; main_model(2,4) = 1; main_model(2,5) = 1; main_model(5,6) = 1; main_model(8,2) = 1; main_model(8,4) = 1; main_model(8,6) = 1; main_model(4,7) = 1; main_model(1,8) = 1; main_model(5,8) = 1;
            ...
            % ------------------------------------------------------------------------------
            if sub <= 9 
                save(['DCM_sub-00', sub, '-session-', cond, '.mat'],'DCM','RCM','BMC','BMA'); 
                save(['GCM_sub-00', sub, '-session-', cond, '.mat'],'DCM'); 
            else
                save(['DCM_sub-0', sub, '-session-', cond, '.mat'],'DCM','RCM','BMC','BMA'); 
                save(['GCM_sub-0', sub, '-session-', cond, '.mat'],'DCM'); 
            end 
